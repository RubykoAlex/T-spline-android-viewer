...
{
public:
	RhParser(const string& filename,  std::istream *fin);
	~RhParser(){};

protected:
	/** Read file. */
	RhTsplinePtr readFile(const string& filename,  std::istream &fin);
public:
...
protected:
	...
	bool extractFace(istream &s);
	/** Extract all edges from file. */
	bool extractEdge(istream &s);
	/** Extract all vertices from file. */
	bool extractVertex(istream &s);
	/** Extract all links from file. */
	bool extractLink(istream &s);
	/** Extract all edgeconditions from file. */
	bool extractEdgeCondition(istream &s);
	/** Extract all meta0 from file. */
	bool extractMeta(istream &s);
	/** Extract all grips from file. */
	bool extractGrip(istream &s);
	/** Extract tolerance from file. */
	bool extractTolerance(istream &s);
	/** Extract version from file. */
	bool extractVersion(istream &s);

	/** Skip one line. */
	void skipLine(istream &s);
	/** Skip blank. */
	void skipBlank(istream &s);
	/** Check if the line has a T-spline element. */
	bool detectLine(istream &s);
private:
