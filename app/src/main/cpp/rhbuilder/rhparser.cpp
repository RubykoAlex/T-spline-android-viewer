...
RhParser::RhParser( const string& filename,  std::istream *fin )
{
	readFile(filename, *fin);
}

RhTsplinePtr RhParser::readFile( const string& filename,  std::istream &fin )
{
	//std::ifstream fin;
	//fin.open(filename);
	string cmd;
...
bool RhParser::extractFace( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractEdge( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractVertex( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractLink( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractEdgeCondition( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractMeta( istream &s )
{
	if (!detectLine(s))
	{
		_rh_tspline->vertices.push_back(0);
...
bool RhParser::extractGrip( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractTolerance( istream &s )
{
	if (!detectLine(s))
	{
...
bool RhParser::extractVersion( istream &s )
{
	if (!detectLine(s))
	{
...
void RhParser::skipLine( istream &s )
{
	char c = s.peek();
	if (c != '\n')
	{
...
void RhParser::skipBlank( istream &s )
{
	char t = s.peek();
	while (t == ' ' || t == '\t')
	{
...
bool RhParser::detectLine( istream &s )
{
	char t = s.peek();
	if (t == ' ' || t == '\t')
	{
