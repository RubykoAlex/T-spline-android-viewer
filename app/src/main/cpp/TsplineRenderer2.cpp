...
// Ctor
//--------------------------------------------------------------------------------
TsplineRenderer::TsplineRenderer() {}
...
TsplineRenderer::~TsplineRenderer() { Unload(); }

void TsplineRenderer::Init() {


  FileReader fileReader = FileReader();
  TSplinePtr spline = fileReader.read_file();
  //load T-spline model
  ...
  const long index_number = (trimesh->sizeTriangles())*3;
  uint16_t  tsplineIndices[index_number];
  ...
  const long pos_number = trimesh->sizePoints()*3;
  float tsplinePositions[pos_number];
  P3dVIterator piter = trimesh->pointIteratorBegin();
  for (int32_t i = 0;piter!=trimesh->pointIteratorEnd();piter++,  i++)
  {
    tsplinePositions[i] = (*piter)->x();
    tsplinePositions[i+1] = (*piter)->y();
    tsplinePositions[i+2] = (*piter)->z();
    i+=2;
  }

  const long norm_number = trimesh->sizeNormals()*3;
  float tsplineNormals[norm_number];
  N3dVIterator niter = trimesh->normalIteratorBegin();
  for (int32_t i = 0;niter!=trimesh->normalIteratorEnd();niter++,  i++)
  {
    tsplineNormals[i] = (*niter)->i();
    tsplineNormals[i+1] = (*niter)->j();
    tsplineNormals[i+2] = (*niter)->k();
    i+=2;
  }
  ...
  // Create Index buffer
  num_indices_ = sizeof(tsplineIndices) / sizeof(tsplineIndices[0]);
  glGenBuffers(1, &ibo_);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo_);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(tsplineIndices), tsplineIndices,
               GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

  // Create VBO
  num_vertices_ = sizeof(tsplinePositions) / sizeof(tsplinePositions[0]) / 3;
  int32_t stride = sizeof(TEAPOT_VERTEX);
  int32_t index = 0;
  TEAPOT_VERTEX* p = new TEAPOT_VERTEX[num_vertices_];
  for (int32_t i = 0; i < num_vertices_; ++i) {
    p[i].pos[0] = tsplinePositions[index];
    p[i].pos[1] = tsplinePositions[index + 1];
    p[i].pos[2] = tsplinePositions[index + 2];

    p[i].normal[0] = tsplineNormals[index];
    p[i].normal[1] = tsplineNormals[index + 1];
    p[i].normal[2] = tsplineNormals[index + 2];
    index += 3;
  }
  glGenBuffers(1, &vbo_);
...
void TsplineRenderer::UpdateViewport() {
...
void TsplineRenderer::Update(float fTime) {
...
bool TsplineRenderer::Bind(ndk_helper::TapCamera* camera) {
  camera_ = camera;
  return true;
}
